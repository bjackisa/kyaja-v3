...
import { useOtherCategories } from "@/hooks/use-categories";

interface DetailedPageProps {
  slug: string;
}

const DetailedPage: React.FC<DetailedPageProps> = ({ slug }) => {
  const { ref, inView } = useInView();
  const { data: otherCategories, isLoading: isFetchingCategories } =
    useOtherCategories(slug);
  const {
    data,
    fetchNextPage,
    hasNextPage,
      ...
      <div className="md:max-w-7xl w-full mx-auto px-4 pb-8 pt-3 md:pt-10">
        ...
        <div className="flex gap-8">
          ...
          <div className="flex-1">
            <CategoryPage department={department} />
