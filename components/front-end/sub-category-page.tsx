...
import { useInfiniteCategoryProducts } from "@/hooks/use-products";
import { useCategoryDetails, useOtherCategoriesExceptCurrentCat } from "@/hooks/use-categories";
import SubCatPage from "./sub-cat";
import { CategoryNav } from "./CategoryNav";
import { FilterSidebar } from "./FilterSidebar";


const CategoryDetailPage: React.FC<{ slug: string }> = ({ slug }) => {
    const { ref, inView } = useInView();
    
    const { data: category, isLoading: isLoadingCategory } = useCategoryDetails(slug);
    const { data: otherCategories, isLoading: isLoading } = useOtherCategoriesExceptCurrentCat(slug);
    ...
    if (status === "pending" || isLoadingCategory || isLoading) {
      return (
           ...
           <div className="md:max-w-7xl w-full mx-auto px-4 pb-8 pt-3 md:pt-10">
             ...
             <div className="flex gap-8">
               ...
               <div className="flex-1">
                 <SubCatPage subcat={category.subCategories} />
  ...
  <div className="no-products-message whitespace-nowrap text-sm">
  ...
  export default React.memo(CategoryDetailPage);
